import { NotebookCellKind, TaskDefinition, type Terminal, TerminalDimensions, Uri, ExtensionContext, NotebookRendererMessaging, NotebookEditor } from 'vscode';
import { z } from 'zod';
import { Bus } from 'tangle';
import { GroupIdentifier, InstanceLifecycleType, InstanceStateName, MonitoringState, _InstanceType } from '@aws-sdk/client-ec2';
...
import { IWorkflowRun, RepositoryEnvironments, WorkflowDispatch } from './extension/services/github/types';
...
export declare namespace Serializer {
    ...
    type Cell = Omit<Grpc.Cell, 'kind' | 'metadata' | 'languageId' | 'outputs' | 'executionSummary'> & {
        metadata?: Metadata;
        kind: NotebookCellKind;
        languageId?: string;
    };
    enum LifecycleIdentity {
        UNSPECIFIED = 0,
        ALL = 1,
        DOCUMENT = 2,
        CELL = 3
...
export declare enum GceActionType {
...
export declare enum InstanceStatusType {
    Stopping = "STOPPING",
...
export type GCPState = GcpGkeClustersState | GcpGkeClusterState | GcpGceVMInstancesState | GcpGceVMInstanceState | GcpCloudRunServicesState | GcpCloudRunRevisionsState;
...
interface Payload {
    ...
    [OutputType.annotations]: AnnotationsPayload & {
        settings: Settings;
    };
    ...
    [OutputType.gcp]?: GcpGkeClusterState | GcpGkeClustersState | GcpGceVMInstancesState | GcpCloudRunServicesState | GcpCloudRunRevisionsState;
...
export type ActiveTerminal = RunmeTerminal & {
    executionId: number;
    runmeId: string;
};
export declare enum NotebookAutoSaveSetting {
    Yes = "yes",
    No = "no"
}
export declare enum EnvVarMode {
    Docs = "docs",
    Shell = "shell"
}
